...
class TransformerConfig(FairseqDataclass):
    ...
    embedding_dump_path: str = field(
        default=".",
        metadata={
            "help": "the path to embedding dumped"
        },
    )
    ...
    shared_graph: bool = field(
        default=False,
        metadata={
            "help": "if set, graph is shared for encoder & decoder"
        },
    )
    sparse_graph: bool = field(
        default=False,
        metadata={
            "help": "if set, use sparse graph."
        },
    )
    hop_num: int = field(
        default=2,
